...
```{r}
# Ihre Lösung:
t_m = t.test(df[df$sex == 1,]$chol, conf.level = 0.95, alternative = 'two.sided')
t_f = t.test(df[df$sex == 0,]$chol, conf.level = 0.95, alternative = 'two.sided')
...
```{r}
# Ihre Lösung:
df$goal = factor(sapply(df$goal, function(x) if (x == 0) 0 else 1))
ggplot(df) +
...
$H_0$: Der Cholesterin-Wert bei erkrankten Patienten (U) ist nicht höher als bei nicht erkrankten (H): $E(H - U) \leq 0$ 
$H_1$: Der Cholesterin-Wert bei erkrankten Patienten ist höher als bei nicht erkrankten $E(H - U) > 0$
...
```{r}
# Ihre Lösung:
t.test(df[df$goal == 0,]$chol, df[df$goal == 1,]$chol, conf.level = 0.95, mu = 0, alternative = 'less')
```

Da p größer als 0,05 ist, kann $H_0$ nicht verworfen werden.
...
```{r}
# Ihre Lösung:
t_negative = t.test(df[df$goal == 0,]$trestbps, conf.level = 0.95, alternative = 'two.sided')
t_positive = t.test(df[df$goal == 1,]$trestbps, conf.level = 0.95, alternative = 'two.sided')
result = data.frame(t_negative$conf.int, t_positive$conf.int)
rownames(result) = c('lower', 'upper')
colnames(result) = c('negative', 'positive')
kable(result)
...
$H_0$: Der Ruhe-Blutdruck bei erkrankten Patienten (U) ist nicht höher als bei nicht erkrankten (H): $E(H - U) \leq 0$ 
$H_1$: Der Ruhe-Blutdruck bei erkrankten Patienten ist höher als bei nicht erkrankten $E(H - U) > 0$
...
```{r}
# Ihre Lösung:
t.test(df[df$goal == 0,]$trestbps, df[df$goal == 1,]$trestbps, conf.level = 0.95, alternative = 'less')
```
