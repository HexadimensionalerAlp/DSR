...
df = data.frame(read.csv(file, sep=',', header=TRUE))
...
```{r}
# Ihre Lösung:
t_m = t.test(df[df$sex == 0,]$chol, conf.level = 0.95)
t_f = t.test(df[df$sex == 1,]$chol, conf.level = 0.95)
...
```{r}
# Ihre Lösung:
warning('Bei Diagnose nur zwei Gruppen?')
ggplot(df) +
  geom_boxplot(aes(goal, chol))
```
...
```{r}
# Ihre Lösung:
t_negative = t.test(df[df$goal == 0,]$chol, conf.level = 0.95)
t_positive = t.test(df[df$goal %in% factor(c(1, 2, 3, 4)),]$chol, conf.level = 0.95)
...
```{r}
# Ihre Lösung:
t.test(df[df$goal %in% factor(c(1, 2, 3, 4)),]$chol, conf.level = 0.95, alternative = 'greater')
```
...
t_positive = t.test(df[df$goal %in% factor(c(1, 2, 3, 4)),]$trestbps, conf.level = 0.95)
result = data.frame(t_negative$conf.int, t_positive$conf.int)
rownames(result) = c('lower', 'upper')
colnames(result) = c('negative', 'positive')
kable(result)
...
Formulieren Sie die Hypothesen ($H_0$ und $H_1$).
$H_0$: Der Ruhe-Blutdruck bei erkrankten Patienten ist nicht höher als bei gesunden $E(X) = \mu \leq 0$
$H_1$: Der Ruhe-Blutdruck bei erkrankten Patienten ist höher als bei gesunden $E(X) = \mu > 0$
...
```{r}
# Ihre Lösung:
t.test(df[df$goal %in% factor(c(1, 2, 3, 4)),]$trestbps, conf.level = 0.95, alternative = 'greater')
```
